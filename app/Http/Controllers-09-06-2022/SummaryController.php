...
class SummaryController extends Controller
{
	...
	*/
    ...
    public function doCreate(Request $request,$project_uuid)
    {
		...
		if($activityname){
			$summary = $summaryArr->summary;
		}else{ 
			$summary = "";
		}
		/*
		echo"<pre>";
		echo $costdetails->activity_name;
		echo $reportData = json_encode($costdetails->report_data);
   ...
   public function summary_filter_array($array,$term){
     ...
     * @return \Illuminate\Http\Response
     */
    public function getSummary($simulation_uuid,$project_uuid,$activity_uuid)
    {
        //$url = $this->baseUrl."/".$this->org_uuid."/cost_summary?simulation_uuid=".$simulation_uuid."&project_uuid=".$project_uuid; 
		if($activity_uuid)
		{
		 $url = $this->baseUrl."/".$this->org_uuid."/cost_summary?simulation_uuid=".$simulation_uuid."&project_uuid=".$project_uuid."&activity_uuid=".$activity_uuid; 
		}else{
		$url = $this->baseUrl."/".$this->org_uuid."/cost_summary?simulation_uuid=".$simulation_uuid."&project_uuid=".$project_uuid; 	
		}
		$method ='GET';				
