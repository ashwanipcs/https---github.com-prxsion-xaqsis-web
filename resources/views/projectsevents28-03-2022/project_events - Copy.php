...
<script type="text/event-calendar" id="event-calendar">
  {{ 
            ...
            <tr>
              ...
              <td style="text-align: right">
                <span class="btn-group">
                  ...
                  <button class= "btn btn-default" data-mode="week">Manage Event</button>
                </span>
              </td>
            </tr>
          </table>
...
(function ($) {
  ...
  var t = $.quicktmpl($('#event-calendar').get(0).innerHTML);
  ...
  function calendar($el, options) {
    ...
    $el.on('click', '.js-cal-prev', function () {
      ...
      for (; y < l; y++) {if (window.CP.shouldStopExecution(0)) break;
    ...
    function monthAddEvent(index, event) {
      ...
      while (e <= event.end && (day.length || endday || options.date < checkanyway)) {if (window.CP.shouldStopExecution(1)) break;
        if (day.length) {
          existing = day.find('.event').length;
          numbevents = Math.max(numbevents, existing);
          for (i = 0; i < numbevents - existing; i++) {if (window.CP.shouldStopExecution(2)) break;
...
event_data = [{title: 'Public Holiday', start: new Date(1647887400000), end: null, allDay: true, text: '22 March, Holiday'},
		  {title: 'Meeting', start: new Date(1647973800000), end: null, allDay: true, text: 'Now we have a meeting'},
		  {title: 'Lockdown', start: new Date(1647973800000), end: null, allDay: true, text: 'Lockdown'}]


for (i = 0; i < event_data.length; i++) {
data.push(event_data[i]);
}
