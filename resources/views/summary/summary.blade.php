                                ...
                                <div class="form-group">
                                    <select name="activity_name" class="form-control projectActivity" id="exampleFormControlSelect2 " required>
										...
										@if($activityProjects)
											...
											<option value="{{$val->name}}" {{($val->activity_uuid == $activityname) ? 'selected' : '' }}>{{$val->name}}</option>
											@endforeach
										@endif
                                      </select>
										...
										<li role="presentation" class="btn btn btn-default"><a href="#example2-tab2" aria-controls="example2-tab2" role="tab" data-toggle="tab">Cost Details</a></li>
												...
												<div class="tab-content">
													<div role="tabpanel" class="tab-pane fade show active" id="example2-tab1">
														<button id="export" class="btn btn-primary">Dwonload CSV</button>
													...
													<div role="tabpanel" class="tab-pane fade" id="example2-tab2">
														<table id="example2-tab2-dt" class="table table-striped table-bordered table-condensed summaryDataTable" cellspacing="0" width="100%">
															<thead>
																<tr>
																	<th>Iteration</th>
																	<th>{{ $costdetails->activity_name}}</th>
																</tr>
															</thead>
															<tbody>
																@if($costdetails)
																	@foreach($costdetails->report_data as $ks => $sum)
																	<tr>
																	<td>{{ $ks+1}}</td>
																		<td>{{ $sum}}</td>							
																	</tr>
...
$( "#export" ).on( "click", function(e) {
    ...
    hiddenElement.download = 'costsummary.csv';
    hiddenElement.click();
});
