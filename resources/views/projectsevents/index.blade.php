						...
						<div class="form-group row">
							...
							@if($projects)
								...
								<div class="checkbox checkbox-success">
								  <input name="project_uuid[]" value="{{$items->uuid}}" id="chkjquery" class="styled projectuiidchk" type="checkbox" >
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
      ...
      nowIndicator: true,
	   select: function(start, end) {
				...
				$(".uuid").val('');
				$(".txtevents").val('0');	
				//$(".projectuiidchk").val('0');	
				$('input[name=project_uuid]').attr('checked', false);
				$('.showModel').modal({
					backdrop: 'static',
					keyboard: false
				});
	  ...
	  eventClick: function(info) {
			...
			$('input[type=checkbox]').each(function () {
				var selChkVal = $(this).val();
				if(selChkVal == info.event.extendedProps.projectuuid)
				{
					$('.projectuiidchk').attr('checked',true);
				}
			});
			...
			$('.frmAction').attr('action', "{{route('projectevents.update')}}");
		  }
    });
