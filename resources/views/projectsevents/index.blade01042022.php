...
@section('head')
   <!-- Prism -->
<link rel="stylesheet" href="{{ url('assets/lib/main.css') }}" type="text/css"> 
<style>
 body {
    margin: 40px 10px;
    padding: 0;
...
<script src="{{ url('assets/lib/main.js') }}"></script>
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
      ...
      events: [
         @foreach($res as $events)
			@php $start_date = date('Y-m-d H:i',$events->start_date); $end_date = date('Y-m-d H:i',$events->end_date); @endphp
			{
				title: '{{$events->name}}',
				start: '{{$start_date}}',
				end: '{{$end_date}}'
