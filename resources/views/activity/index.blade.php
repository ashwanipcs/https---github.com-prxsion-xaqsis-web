                ...
                <div class="card">						
					...
					<table class="table table-striped table-bordered nowrap showDatatable" cellspacing="0" width="100%">
                            ...
                            <tr>
                                ...
                                <td> 
									<!--<a href="javascript:void(0); return false;" data-id="{{$item->uuid}}/@ASVAY@/{{$item->name}}/@ASVAY@/{{$item->description}}" class="editModel btn btn-success btn-floating" data-toggle="tooltip" data-placement="top" title="Edit">
									 <i class="fa fa-edit"></i></a>-->
									 <a href="javascript:void(0);"  data-uuid="{{$item->uuid}}" data-name="{{$item->name}}" data-desc="{{$item->description}}" data-active="{{$item->is_active}}" class="editModel btn btn-success btn-floating" data-toggle="tooltip" data-placement="top" title="Edit">
									 <i class="fa fa-edit"></i></a>
										
									<a href="javascript:void(0);"  data-id="{{route('activity.remove', $item_id)}}" class="confirmDelete btn btn-danger btn-floating"  data-toggle="tooltip" data-placement="top" title="Delete">
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<table class="table table-striped table-bordered nowrap" cellspacing="0" border="1px;">                        
                            ...
                            <tr>
                                <td class="text-center"><input name="chk[]" type="checkbox" value="{{$ac->uuid}}" class="chkInput form-check-input"></td>
								<td class="text-center"><input type="text" name="name[]" value="{{$ac->name}}" class="form-control"></td> 
								<td class="text-center"><input type="text" name="description[]" value="{{$ac->description}}" class="form-control"></td>                                 
                                <td class="text-center">
									<div class="form-check">
										<input name="is_active[]" type="checkbox" id="gridCheck1" class="form-check-input"  {{($item->is_active == true ? ' checked' : '') }}>
									</div>
								</td>                                
							@endforeach  
							@endif
...
$(document).ready(function(){
	$("body").on("click", ".editModel", function(event){ 		 
		...
		$('.frmAction').attr('action', "{{route('activity.update')}}");		 	
...
$(document).ready(function(){
	$('.importBtn').click(function(){		
		...
		$('.frmAction').attr('action', "{{route('activity.syncActivity')}}");		
