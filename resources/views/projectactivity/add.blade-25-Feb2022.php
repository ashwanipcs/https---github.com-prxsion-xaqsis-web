								...
								<th style="width:10%">Actions</th>
								 ...
								 @if($projectactivity)
									...
									@php $item_id = Crypt::encrypt($items->uuid); @endphp
											...
											<td style="float: right;margin: 11px 9px 9px 11px;"><input name="is_active" class="form-check-input" type="checkbox" id="gridCheck1" @if($items->is_active==1) checked @else "" @endif></td>													 
											<td> 
...
$(document).ready(function(){
	$(".addCF").click(function(){
				...
				@foreach($activity as $k=>$val)																
				htmlOpt +='<option value="{{$val->uuid}}">{{$val->name}}</option>';															    
				@endforeach                                                
				htmlOpt +='</select>';
				htmlOpt +='</td>';
				...
				htmlOpt +='<td style="float: right;margin: 11px 9px 9px 11px;"><input style="border: white;" name="is_active" class="form-check-input" type="checkbox"></td>';													 
				htmlOpt +='<td>';
				htmlOpt +='<a href="javascript:void(0);" class="remCF">';
				htmlOpt +='<img style="height:20px;" src="{{ url('public/assets/media/image/icons/delete.png') }}" alt="logo">';
				htmlOpt +='</a>';
				htmlOpt +='</td>';
				htmlOpt +='</tr> ';
