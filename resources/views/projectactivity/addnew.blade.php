...
@section('content')
            ...
            <div class="col-md-12">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
						...
						<form name="frmPrActivity" id="updateproactivity" method="post" action="{{route('projectactivity.updateprojectactivity')}}">
						@csrf
						  <table id="sortable" class="table table-striped table-bordered nowrap showDatatable" cellspacing="0" width="100%">
							...
							@if($projectactivity)
								...
								@php $item_id = Crypt::encrypt($items->uuid); 
									...
									$string = '';
									foreach($items->predecessors_uuid as $k=>$v)
									{
										$string .= ',"'.$v.'"';
									}
									$stringListValue = "[".substr($string, 1)."]";	
								@endphp
								<tr data-rowid="{{$items->uuid}}">
								  <td>{{$items->activity_index}}
									<input type="hidden" name="tableIndex" class="form-control tableindex" value="{{$items->activity_index}}">									
								  </td>
								  <td>{{$items->name}}
									<input type="hidden" name="tableuuid" class="form-control tableuuid" value="{{$items->uuid}}">
								  </td>								 
								  ...
								  <td>
									...
									data-pd="{{$items->pessimistic_duration}}" data-prededata="{{$stringListValue}}" data-predecessors="{{$stringListValue}}" class="editModel" data-toggle="tooltip" data-placement="top" title="Edit">
									Edit</a> | <a href="{{route('projectactivity.remove', $items->uuid)}}" class="confirm" id="checkConfirmation">Remove</a>
									<!-- <a href="{{route('projectactivity.remove', $items->uuid)}}" class="delete"><i class="fa fa-trash"></i></a>-->
	...
	<div class="modal fade showModel" role="dialog">
		<div class="modal-dialog modal-xl">
			<!-- Modal content-->
			<form method="post" class="frmAction needs-validation" novalidate>
			@csrf
						...
						<input type="hidden" name="project_uuid" class="form-control projectid" value="">					 
						  ...
						  <div class="form-row">
							<div class="col-md-4 mb-3">
							  ...
							  <input type="number" name="mostlikely_cost" class="form-control mostlikelycost" id="validationCustom01"  value="0.00" required>							
							</div>
							<div class="col-md-4 mb-3">
							  <label for="validationCustom02">Optimistic Cost</label>
							  <input type="number" name="optimistic_cost" class="form-control optimisticcost" id="validationCustom02"  value="0.00" required>							  
							</div>
							<div class="col-md-4 mb-3">
							  <label for="validationCustomUsername">Pessimistic Cost</label>
							  <div class="input-group">
								<input type="number" name="pessimistic_cost" class="form-control pessimisticcost" id="validationCustomUsername" value="0.00"  required>
							  </div>
							</div>
						  </div>
						  ...
						  <div class="form-row">
							<div class="col-md-4 mb-3">
							  ...
							  <input type="number" name="mostlikely_duration" class="form-control mostlikelyduration" id="validationCustom01"  value="0" required>
							</div>
							<div class="col-md-4 mb-3">
							  <label for="validationCustom02">Optimistic Duration</label>
							  <input type="number" name="optimistic_duration" class="form-control optimisticduration" id="validationCustom02"  value="0" required>
							</div>
							<div class="col-md-4 mb-3">
							  <label for="validationCustomUsername">Pessimistic Duration</label>
							  <div class="input-group">
								<input type="number" name="pessimistic_duration" class="form-control pessimisticduration" id="validationCustomUsername" value="0" required>
							  </div>
							</div>
						  </div>
	...
	<div class="modal fade updateShowModel" role="dialog">
		<div class="modal-dialog modal-xl">
			<!-- Modal content-->
			<form method="post" class="frmUpdateAction needs-validation" novalidate>
			@csrf
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal">&times;</button>
...
<script src="{{ url('public/vendors/jquery/jquery-ui.min.js') }}"></script> -->
<!-- Javascript -->
...
$(document).ready(function() {
	...
	table.on( 'row-reorder', function ( e, diff, edit ) {
		...
		let idQ = $(this).data('rowid');
		//let idNewQ = idQ.replace("row_", "");
		let idNewQ = idQ;
		 // get position
		 let position = diff[i].newPosition+1;
		 alert("idNewQ=="+idNewQ+" position=="+position);
   ...
   $('#updateproactivity').on('submit', function(e){
      var form = this;
      // Encode a set of form elements from all pages as an array of names and values
      var params = table.$('input,hidden').serializeArray();
 ...
 function updateOrder(idQuestion, order){
      $.get("{{url('/')}}/projectactivity/updateOrder/", { question_id: idQuestion, order: order } )
      .done(function( data ) {
        console.log(data);
      });
    }
...
$("body").on("click", ".editModel", function(event){	
	...
	var prededata = $(this).data('prededata');	
	//alert(predecessors+" =prededata="+prededata);
...
$(function() {
    $('.confirm').click(function(e) {
        e.preventDefault();
        if (window.confirm("Are you sure want to remove thsi items?")) {
