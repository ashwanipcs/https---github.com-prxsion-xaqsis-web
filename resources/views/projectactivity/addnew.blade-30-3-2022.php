            ...
            <div class="col-md-12">
                <div class="row">
                    <div class="col-md-12 text-right mb-3" >                        
						<a href="javascript:void(0);" class="addmodel btn btn-primary btn-sm rounded-0" data-toggle="tooltip" data-placement="top" title="Add">Add</a>
						  ...
						  <table id="sortable">
							...
							@if($projectactivity)
								...
								<tr>
								  ...
								  <td>
									 <a href="javascript:void(0);" data-uuid="{{$items->uuid}}" data-activityuuid="{{$items->activity_uuid}}" data-mc="{{$items->mostlikely_cost}}" data-oc="{{$items->optimistic_cost}}" 
									 data-pc="{{$items->pessimistic_cost}}" data-md="{{$items->mostlikely_duration}}" data-od="{{$items->optimistic_duration}}"
									data-pd="{{$items->pessimistic_duration}}" class="editModel btn btn-success btn-floating" data-toggle="tooltip" data-placement="top" title="Edit">
		...
		<div class="modal-dialog modal-xl">
			...
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">Project Activity</h4>
				</div>
				<div class="modal-body">	
						<input type="text" name="project_uuid" class="form-control projectid" value="">					 
						  <div class="form-row">
								...
								<label for="validationCustom01">Projects Activity</label>								 
									<select name="activity_uuid" class="form-control" id="exampleFormControlSelect1" required>
									   ...
									   @foreach($activity as $k=>$val)
										<option value="{{$val->uuid}}" @if($val->uuid == $items->activity_uuid)?selected:'' @endif>{{$val->name}}</option>	
									   @endforeach                                                
								  </select>	
								...
								<select name="predecessors[]" class="select-parent-activity" multiple>
								  ...
								  <optgroup label="Activity">
									 @foreach($activity as $k1=>$val1)																
										<option value="{{$val1->uuid}}" @if($val1->uuid == $items->activity_uuid) selected @else  @endif>{{$val1->name}}</option>@endforeach  
								  </optgroup>								 
								</select>
						</div>
						</div>
				...
				<div class="modal-body">	
						  ...
						  <div class="form-row">
							<div class="col-md-12 mb-3">
								<label for="validationCustom01">Projects Activity</label>
								<input type="text" name="project_uuid" class="form-control projectid" value="">
								<input type="text" name="activity_uuid" class="form-control activityuuid" value="">
