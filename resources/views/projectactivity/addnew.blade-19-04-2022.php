						...
						@csrf
						  <table id="sortable" class="table table-striped table-bordered nowrap showDatatable" cellspacing="0" width="100%">
							...
							@if($projectactivity)
								...
								@endphp
								<tr>
								  <td data-id="{{$items->activity_index}}">{{$items->activity_index}}
									<input type="hidden" name="tableIndex[]" class="form-control" value="{{$items->activity_index}}">									
								  </td>
								  <td>{{$items->name}}
									<input type="hidden" name="tableuuid[]" class="form-control" value="{{$items->uuid}}">
								  </td>								 
								  ...
								  <td>
									...
									data-pd="{{$items->pessimistic_duration}}" data-prededata="{{$stringListValue}}" data-predecessors="{{$stringListValue}}" class="editModel btn btn-success btn-floating" data-toggle="tooltip" data-placement="top" title="Edit">
									<i class="fa fa-edit"></i></a>
									 <a href="{{route('projectactivity.remove', $items->uuid)}}"><i class="fa fa-trash"></i></a>
									</td>
								</tr>
								@endforeach
							@else	
